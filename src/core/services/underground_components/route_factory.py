...
class RouteFactory:
    ...
    def create_cross_country_route(self, from_station: str, to_station: str) -> Optional[Route]:
        ...
        """
        if not self.geographic_utils.is_cross_country_route(from_station, to_station):
            return None
        ...
        from_region = self.geographic_utils._get_station_region(from_station)
        to_region = self.geographic_utils._get_station_region(to_station)
        
        # Determine appropriate terminals based on regions
        southern_terminals = self.geographic_utils.get_region_terminals("South England")
        northern_terminals = self.geographic_utils.get_region_terminals("Scotland")
        ...
        if from_region == "South England" and to_region == "Scotland":
            ...
            southern_terminus = self.geographic_utils.find_best_terminus_for_station(from_station, southern_terminals, self.data_repository)
            
            # Find best northern terminus for destination
            northern_terminus = self.geographic_utils.find_best_terminus_for_station(to_station, northern_terminals, self.data_repository)
            
            # Find best London Underground connection
            london_from, london_to = self.geographic_utils.find_best_london_connection(southern_terminus, northern_terminus)
            ...
            if from_station != southern_terminus:
                segment1 = self._create_national_rail_segment(
                    from_station,
                    southern_terminus,
                    self.journey_estimator.get_line_between_stations(from_station, southern_terminus, self.data_repository)
                )
            ...
            segment2 = RouteSegment(
                ...
                distance_km=self.journey_estimator.estimate_underground_distance(london_from, london_to, "london"),
                journey_time_minutes=self.journey_estimator.estimate_underground_time(london_from, london_to, "london"),
                service_pattern="UNDERGROUND",
            ...
            segment3 = self._create_national_rail_segment(
                london_to,
                northern_terminus,
                self.journey_estimator.get_line_between_stations(london_to, northern_terminus, self.data_repository)
            )
            ...
            if to_station != northern_terminus:
                ...
                system_info = self.station_classifier.get_underground_system(to_station)
                if system_info:
                    system_key, system_name = system_info
                    ...
                    segment4 = RouteSegment(
                        ...
                        to_station=to_station,
                        line_name=system_name,
                        distance_km=self.journey_estimator.estimate_underground_distance(northern_terminus, to_station, system_key),
                        journey_time_minutes=self.journey_estimator.estimate_underground_time(northern_terminus, to_station, system_key),
                        service_pattern="UNDERGROUND",
                        train_service_id=f"{system_key.upper()}_UNDERGROUND_SERVICE"
                    )
        ...
        # For North to South (e.g., Glasgow to Southampton)
            ...
            northern_terminus = self.geographic_utils.find_best_terminus_for_station(from_station, northern_terminals, self.data_repository)
            
            # Find best southern terminus for destination
            southern_terminus = self.geographic_utils.find_best_terminus_for_station(to_station, southern_terminals, self.data_repository)
            
            # Find best London Underground connection
            london_from, london_to = self.geographic_utils.find_best_london_connection(northern_terminus, southern_terminus)
            ...
            if from_station != northern_terminus:
                ...
                system_info = self.station_classifier.get_underground_system(from_station)
                if system_info:
                    system_key, system_name = system_info
                    ...
                    segment1 = RouteSegment(
                        ...
                        line_name=system_name,
                        distance_km=self.journey_estimator.estimate_underground_distance(from_station, northern_terminus, system_key),
                        journey_time_minutes=self.journey_estimator.estimate_underground_time(from_station, northern_terminus, system_key),
                        service_pattern="UNDERGROUND",
                        train_service_id=f"{system_key.upper()}_UNDERGROUND_SERVICE"
                    )
                    segments.append(segment1)
            ...
            segment2 = self._create_national_rail_segment(
                ...
                london_from,
                self.journey_estimator.get_line_between_stations(northern_terminus if segments else from_station, london_from, self.data_repository)
            )
            segments.append(segment2)
            ...
            segment4 = self._create_national_rail_segment(
                london_to,
                southern_terminus,
                self.journey_estimator.get_line_between_stations(london_to, southern_terminus, self.data_repository)
            )
            segments.append(segment4)
            ...
            if to_station != southern_terminus:
                segment5 = self._create_national_rail_segment(
                    southern_terminus,
                    to_station,
                    self.journey_estimator.get_line_between_stations(southern_terminus, to_station, self.data_repository)
                )
